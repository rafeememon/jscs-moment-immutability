...
function hasClone (node) {
  if (node.type === 'CallExpression') {
    if (node.callee.type === 'MemberExpression' &&
      ...
      return node.callee.object && hasClone(node.callee.object)
    }
  } else {
    return false
  }
...
function hasConstructor (node) {
  if (node.type === 'CallExpression') {
    if (node.callee.type === 'Identifier' &&
        node.callee.name === 'moment') {
      return true
    } else {
      return node.callee.object && hasConstructor(node.callee.object)
    }
  } else {
    return false
  }
...
function isSafeMutation (node) {
  return hasClone(node) || hasConstructor(node)
}
...
module.exports.prototype = {
  ...
  check: function (file, errors) {
    file.iterateNodesByType(['CallExpression'], function (node) {
      if (isMutator(node) && !isSafeMutation(node)) {
